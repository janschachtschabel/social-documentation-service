/**
 * Automatisches Datenbank-Setup Script
 * Führt Schema aus und erstellt Test-Benutzer
 * 
 * Verwendung: npm run setup:db
 */
...
async function executeSqlFile(filePath: string): Promise<boolean> {
  console.log(`\n📄 Führe SQL-Datei aus: ${filePath}`);
  
  try {
    const sql = fs.readFileSync(filePath, 'utf-8');
    
    // Supabase SQL API verwenden (wenn verfügbar) oder manuell ausführen
    // Da Supabase JS Client keine direkte SQL-Ausführung unterstützt,
    // müssen wir die Tabellen einzeln prüfen
    
    console.log('ℹ️  Schema-Datei gelesen. Prüfe Tabellen...');
    ...
    console.error('❌ Fehler beim Lesen der SQL-Datei:', error);
    return false;
  }
}
...
async function main() {
  ...
  const schemaExists = await executeSqlFile(schemaPath);
